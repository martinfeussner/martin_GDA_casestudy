...
data_table[data_table$month>=6,"month_c"] = paste("21/", data_table[data_table$month>=6,"month_c"], sep="")
data_table[data_table$month<6,"month_c"] = paste("22/", data_table[data_table$month<6,"month_c"], sep="")
...
ggplot(data_table, aes(x = month_c, y = count, fill = member_casual)) + 
  ...
  scale_y_continuous(labels = scales::percent_format()) + 
  geom_hline(yintercept=customer_type_ratio, linetype=2) +
  labs(title = "Monthly Proportioned Breakdown", x = "Month (yy/mm)", y = "Proportion", fill = "Customer Type") + 
  annotate(geom = "text", x ="22/03", y = customer_type_ratio+0.04, label = "Customer Type Ratio")

ggplot(data_table, ) + 
  geom_bar(aes(x = month_c, fill = member_casual), width=0.6) + 
  labs(title = "Monthly Count Breakdown", x = "Month (yy/mm)", fill = "Customer Type")
